...
export function CreateRecipeScreen({ navigation, route }: Props) {
  ...
  const copyFromId = route.params?.copyFromId;
  const source = copyFromId ? getRecipeById(copyFromId) : undefined;
  ...
  useEffect(() => {
    ...
    setFestivity(source.festivity);
    setPrepTimeMinutes(String(source.prepTimeMinutes));
    setIngredients(source.ingredients.length ? [...source.ingredients] : ['']);
    setMaterials(source.materials.length ? [...source.materials] : ['']);
    setSteps(
      source.steps.length
        ? source.steps.sort((a, b) => a.order - b.order).map((s) => s.instruction)
        : [''],
    );
  }, [source]);
  ...
  const handleSave = () => {
    ...
    Alert.alert('Receita criada!', `"${recipe.title}" foi salva.`, [
      { text: 'Ver receita', onPress: () => navigation.replace('Recipe', { recipeId: recipe.id }) },
    ]);
  };

  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.heading}>Nova receita</Text>
      ...
      <PrimaryButton title="Salvar receita" onPress={handleSave} style={styles.saveButton} />
    </ScrollView>
  );
}
